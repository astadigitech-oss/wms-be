...
class NonDocumentExport implements FromQuery, WithHeadings, WithMapping, ShouldAutoSize, WithStyles
{
    use Exportable;

    protected $nonDocumentId;
    protected $document;

    public function __construct($nonDocumentId)
    {
        $this->nonDocumentId = $nonDocumentId;


        $this->document = NonDocument::find($nonDocumentId);
    }
    ...
    public function query()
    {
        ...
        $displayQuery = New_product::select($commonColumns)
            ...
            ->whereHas('nonDocuments', function ($q) {
                $q->where('non_document_id', $this->nonDocumentId);
            });
