...
class AllNonProductDocumentExport implements WithMultipleSheets
{
    use Exportable;
    ...
    {
        return [
            'All Products' => new AllNonProductsSheet(),
            'Documents Summary' => new AllNonDocumentsSummarySheet(),
        ];
    }
}

class AllNonProductsSheet implements \Maatwebsite\Excel\Concerns\FromQuery, \Maatwebsite\Excel\Concerns\WithHeadings, \Maatwebsite\Excel\Concerns\WithMapping, \Maatwebsite\Excel\Concerns\ShouldAutoSize, \Maatwebsite\Excel\Concerns\WithStyles
{
    ...
    private function loadDocumentStatuses()
    {
        ...
        $displayStatuses = DB::table('non_document_items')
            ->join('non_documents', 'non_document_items.non_document_id', '=', 'non_documents.id')
            ->where('productable_type', New_product::class)
            ->select('productable_id', 'non_documents.status')
            ->get();
        ...
        // 2. Staging
        $stagingStatuses = DB::table('non_document_items')
            ->join('non_documents', 'non_document_items.non_document_id', '=', 'non_documents.id')
        ...
        // 3. Migrate
        $migrateStatuses = DB::table('non_document_items')
            ->join('non_documents', 'non_document_items.non_document_id', '=', 'non_documents.id')
    ...
    public function map($row): array
    {
        ...
        $nonDocStatus = $this->documentStatuses[$cacheKey] ?? 'N/A';
        ...
        if (!empty($row->new_quality)) {
            ...
            if (is_array($qualityData)) {
                if (!empty($qualityData['non'])) $qualityDescription = $qualityData['non'];
...
class AllNonDocumentsSummarySheet implements \Maatwebsite\Excel\Concerns\FromCollection, \Maatwebsite\Excel\Concerns\WithHeadings, \Maatwebsite\Excel\Concerns\ShouldAutoSize, \Maatwebsite\Excel\Concerns\WithStyles
{
    public function collection()
    {
        return NonDocument::with('user:id,name')
            ->orderBy('created_at', 'desc')
            ->get()
            ->map(function ($doc) {
                return [
                    'code_document_non' => $doc->code_document_non,
